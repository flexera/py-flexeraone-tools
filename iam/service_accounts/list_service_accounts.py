...
def list_iam_service_accounts(refresh_token, host, org_id):
    """
    ...
    list_service_accounts(host, access_token, org_id)
...
def list_service_accounts(host, access_token, org_id):
    """
    ...
    list_service_accounts_request = requests.get(service_account_url, **kwargs, stream=True)
    list_service_accounts_request.raise_for_status()
    pprint.pprint(list_service_accounts_request.json())


if __name__ == '__main__':
    # click passes no args
    # pylint: disable=no-value-for-parameter
    list_iam_service_accounts(auto_envvar_prefix='FLEXERA')
